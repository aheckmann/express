...
var multipart = require('multipart'),
    ...
    plugin = require('express/plugin'),
    dsl = require('express/dsl'),
...
Route = Class({
  ...
  normalize: function(path) {
    ...
    return new RegExp('^' + RegExp.escape(request.normalizePath(path), '.')
      .replace(/\*/g, '(.+)')
...
Server = Class({
  ...
  route: function(request, response){
    var self = this,
        request = new request.Request(request, response) 
...
dsl.configure(function(){
  plugin.use(require('express/plugins/view').View)
  plugin.use(require('express/plugins/cache').Cache)
  plugin.use(require('express/plugins/redirect').Redirect)
  plugin.use(require('express/plugins/body-decoder').BodyDecoder)
})

dsl.configure('development', function(){
  dsl.enable('helpful 404')
  dsl.enable('show exceptions')
})

dsl.configure('test', function(){
  dsl.enable('throw exceptions')
})

dsl.configure('production', function(){
  dsl.enable('cache view contents')
  dsl.enable('cache static files')
})
