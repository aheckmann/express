...
Server.prototype.run = function(port, host){
  if (port !== undefined) this.port = port
  if (host !== undefined) this.host = host
  this.listen(this.port, this.host)
  process.addListener('uncaughtException', function (err) { 
    try{
      Express.server.emit('uncaughtException', err)
    } catch(error) {
...
Server.prototype.route = function(request){
...
/**
 * Handle incoming requests from Node
 * @api private
 */
 
function expressRequestListener (req, response) {
...
Express = { 
  ...
  settings: {},
  params: {},
  server: new Server(expressRequestListener)
}
